...
# training dataset
#----------------------------------------------------------------------------------------
def img_datasets(nrrds, IDs, labels, fn_arr_1ch, fn_arr_3ch, fn_df, crop_shape, return_type1, 
                  return_type2, interp_type, input_channel, output_size, output_dir, norm_type):
    ...
    for nrrd_file, patient_id in nrrds, IDs:
        count += 1
        print(count)
    ...
    print(img_df[0:100])
    img_df.to_csv(os.path.join(data_pro_dir, fn_df))
    print('data size:', img_df.shape[0])

#----------------------------------------------------------------------------------------
# training dataset
#----------------------------------------------------------------------------------------
def get_img_datasets(data_tot, ID_tot, label_tot, fns_arr_1ch, fns_arr_3ch, fns_df, crop_shape,
             return_type1, return_type2, interp_type, input_channel, output_size, output_dir,
             norm_type):
    ...
    fn_arr_1chs = ['train_arr_1ch.npy', 'val_arr_1ch.npy', 'test_arr_1ch.npy']
    fn_arr_3chs = ['train_arr_3ch.npy', 'val_arr_3ch.npy', 'test_arr_3ch.npy']
    fn_dfs = ['train_df.csv', 'val_df.csv', 'test_df.csv']
    
    for nrrds, patient_id, labels, fn_arr_1ch, fn_arr_3ch, fn_df in zip(
